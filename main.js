...
var sc = {tx(){ ctx.font = '20px Arial';
ctx.fillText('Score : '+ ten,80,30)    } }; 
var life = {tx(){ ctx.font = '20px Arial';
    ctx.fillText('Life : '+ fe,250,30)  } }; 
var death = {tx(x){ ctx.font = '30px Arial';
    ctx.fillText('G A M E  O V E R',x,162)  } };
var death1 = {tx(x){ ctx.font = '27px Arial';
    ctx.fillText('You have lost all your lives.',x,162)  } };

class Dino{constructor(img2,img4){ this.x=80;this.y=280;this.wid=80;
...
class Cactus{constructor(f,d,id,r,img,w){
...
var cactus= new Cactus(280,80,'cactus',1400,img1,70);
 ...
 cacts.push(cactus,cactus1,clo,ud)};
 cacts.forEach((a,i,o)=>{ a.x-=3; a.draw();
 if(a.x<0){o.splice(i,1); if(a.id==='cactus1'){ten += 10;
       if(ten % 100 === 0) { point.play()  }  } }; 
if(a.id === 'cactus1' || a.id === 'cactus'){ 충돌(dino,a) } });
if (isDead){dino.curr = dino.img4;dino.wid=70; dino.draw();
if (window.innerWidth <= 968 && fe>=0){
    death.tx(250);esh.draw()}else if(window.innerWidth>968 && fe>=0){death.tx(600);esh.x=686;esh.draw()}
if (window.innerWidth <= 968 && fe<0){
    death1.tx(200)}else if(window.innerWidth>968 && fe<0){death1.tx(600)}
}else {dino.draw()}; 

   if (점프중 == true && dino.y>150){ dino.y-=3;  if (!soundPlayed) {
    jumpSound.play(); soundPlayed = true};
}                                
if(dino.y<=150){if(dino.y < 280){ dino.y += 2;console.log('없어도됨');}} 
...
canvas.addEventListener('click', (e)=> { 
    ...
    if (x >= esh.x && x <= esh.x + esh.width && y >= esh.y && y <= esh.y + esh.height) { location.reload(); }
});
var dead = new Audio('die.wav');
function 충돌(dino, cactus) {x축=cactus.x-(dino.x+dino.wid);
                y축=cactus.y-(dino.y+dino.hei);
 last=(cactus.x+cactus.width)-dino.x                          
 if(x축<-48 && (y축<-35 && last>30) ){ dead.play(); fe -= 1;
    localStorage.setItem('mi', fe);  isDead=true; 
    cancelAnimationFrame(animate)} }
var fe = localStorage.getItem('mi'); 
if (fe == null || fe < 0 ) {fe = 3} else {  fe = Number(mi);}
